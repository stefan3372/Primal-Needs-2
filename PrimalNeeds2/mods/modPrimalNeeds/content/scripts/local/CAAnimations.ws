...
class CompleteAnimations {
	...
	function ShallowAnim()
	{
		if (thePlayer.IsInShallowWater() && !PerformingAnimation()) {
			anim = PNA_shallow;
			SheatheSwordIfUnsheatedAndPlayAnimation();
		}
	}
...
state CAAnimation in CR4Player
{
	...
	{
		var drinksp, eatsp, oilsp, grindstonesp, workbenchsp, lootsp, dblootsp, peesp, poopsp, shallowsp, collapsesp : float ;
		...
		switch (anim) 
		{
			...
			case CAT_workbench:
				...
				break;
			case PNA_shallow:
				shallowsp = CAShallowSpeed();
				start = 'man_work_standing_splashing_his_face_start'; startSpeed = shallowsp ;
				loop = 'man_work_standing_splashing_his_face_loop_03'; loopSpeed = shallowsp ;
				stop = 'man_work_standing_splashing_his_face_stop'; stopSpeed = shallowsp ;
				break;
			case PNA_collapse:
				collapsesp = CACollapseSpeed();
				...
				parent.AddTimer('PN_Collapse', 11.0 / collapsesp );
				break;
			case PNA_pee:
				peesp = CAPeeSpeed();
				...
				parent.AddTimer('PN_EndPee', 7.0 / peesp );
				break;
			case PNA_poop:
				poopsp = CAPoopSpeed();
				...
				parent.AddTimer('PN_EndPoop', 5.0 / poopsp );
				break;
			default:
				break;
		}
...
state CAInterruption in CR4Player
{
	...
	function CARemoveAllTimers() {
		...
		parent.RemoveTimer('PN_EndPoop');
		parent.RemoveTimer('PN_EndPee');
		parent.RemoveTimer('PN_Collapse');
